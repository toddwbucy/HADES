//! Viewer connection admission retained through response writes.
//! Kept local to preserve the frontend CLI-contract-only crate boundary.
...
/// Absolute lifetime bounds stalled request/response I/O.
const CONNECTION_LIFETIME: std::time::Duration = std::time::Duration::from_secs(120);
    ...
    async fn accept(&mut self) -> (Self::Io, Self::Addr) {
        loop {
            ...
            if let Some(permit) = try_connection() {
                return (AdmittedIo::new(stream, permit, CONNECTION_LIFETIME), addr);
            }
    ...
    fn check_deadline(&mut self, cx: &mut std::task::Context<'_>) -> std::io::Result<()> {
        ...
        if self.deadline.as_mut().poll(cx).is_ready() {
            Err(std::io::Error::new(
                ...
                "viewer connection lifetime expired",
            ))
        } else {
            Ok(())
        }
