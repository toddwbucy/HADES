...
mod envelope_route_tests {
    ...
    #[tokio::test]
    async fn private_backend_failure_envelope_is_rejected() {
        let _fixture = crate::backend_process::SCRIPT_FIXTURE.lock().await;
        let root = tempfile::tempdir().unwrap();
        let bin = root.path().join("backend");
            ...
            if success {
                ...
                assert!(
                    data["error"]
                        .as_str()
                        .unwrap()
                        .contains("backend reported failure")
                );
                assert!(!String::from_utf8_lossy(&body).contains("synthetic backend failure"));
                assert!(!String::from_utf8_lossy(&body).contains("synthetic-graph"));
            }
        }
    }
}
