...
mod handlers {
    ...
    pub async fn db_aql(
        ...
        // Reject mutating AQL — this handler is read-only. String literals and
        // comments are stripped first so keywords inside them, like the value
        // "INSERT TITLE", are not false positives.
        if let Some(keyword) = mutating_aql_keyword(aql) {
            return Err(HandlerError::InvalidParameter {
                name: "aql".into(),
                reason: format!(
    ...
    fn strip_aql_strings_and_comments(aql: &str) -> String {
        ...
        while i < len {
            ...
            // Non-ASCII bytes become one space each rather than one `char`
            // each. `b as char` would widen a byte >= 0x80 into a multi-byte
            // UTF-8 sequence, so the output's byte offsets would no longer line
            // up with the input's and slicing the result could land mid-char.
            // AQL keywords are ASCII, and a non-ASCII identifier has to be
            // backtick-quoted, which is already blanked above.
    ...
    /// Does `aql` contain a data-modification keyword as a real token?
    ///
    /// Returns the offending keyword, or `None` for a read-only query.
    ///
    /// The predecessor split on whitespace and compared whole words, which is
    /// not AQL tokenization and let two valid mutating forms through. Confirmed
    /// against a live server via `POST /_api/explain`, which plans without
    /// executing: `FOR d IN c REMOVE(d) IN c` yields a RemoveNode and
    /// `INSERT{"a":1} INTO c` yields an InsertNode. Neither token is
    /// whitespace-delimited, so neither was ever seen.
    ///
    /// This scans identifier tokens instead, which is what AQL's own lexer
    /// splits on. `REMOVE(d)` yields `REMOVE`. Two forms are deliberately NOT
    /// keywords and are skipped by their sigil:
    ///
    /// - `d.remove` is attribute access. AQL keywords are reserved, so a bare
    ///   `remove` after `.` can only be a field name.
    /// - `@remove` is a bind parameter name, resolved before planning.
    ///
    /// A collection actually named `remove` must be backtick-quoted in AQL, and
    /// [`strip_aql_strings_and_comments`] has already blanked backtick spans.
    ///
    /// The engine-authoritative check is `_api/explain` plus a scan for
    /// modification nodes, which costs a round trip. It is not used here because
    /// the guard must reject before any server contact, and because the tests
    /// covering it run with no database. The successor backend does not need
    /// either approach: `BEGIN TRANSACTION READ ONLY` makes the defect class
    /// unrepresentable.
    fn mutating_aql_keyword(aql: &str) -> Option<&'static str> {
        const MUTATING: [&str; 5] = ["INSERT", "UPDATE", "REPLACE", "REMOVE", "UPSERT"];
        ...
        while i < bytes.len() {
            ...
            // A token preceded by '.' or '@' is a field name or a bind
            // parameter, never a keyword.
            let sigil = i
                .checked_sub(1)
                .map(|p| bytes[p] == b'.' || bytes[p] == b'@')
                .unwrap_or(false);
            ...
            let token = &stripped[start..i];
            if let Some(kw) = MUTATING.iter().find(|kw| token.eq_ignore_ascii_case(kw)) {
                return Some(kw);
            }
        }
...
mod tests {
    ...
    ///
    /// Both are valid AQL and both plan to a modification node. Confirmed
    /// against a live server with `POST /_api/explain`, which plans without
