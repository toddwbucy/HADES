...
/// Locate a CUDA toolkit for clang to parse against.
///
/// This was hardcoded to `/usr/local/cuda`, and the consequence was silent
/// rather than loud. On a host installing CUDA elsewhere, clang fails with
/// "cannot find CUDA installation", the parse degrades, `<<<...>>>` never
/// resolves into a kernel call, and extraction still reports success with
/// every launch edge missing. Measured on a real file: 30 launches present,
/// 48 symbols recovered, 0 launch edges.
///
/// There is deliberately no fallback to `-nocudainc`. Without CUDA headers
/// `__global__` is not even a known type name, so kernels stop being
/// recognised at all — a worse failure, and equally quiet.
///
/// `CUDA_PATH` is honoured first because it is the toolkit's own convention.
fn cuda_path() -> Option<String> {
    if let Ok(p) = std::env::var("CUDA_PATH") {
        ...
        warn!(
            cuda_path = %p,
            "CUDA_PATH is set but is not a directory; falling back to probing"
        );
    }
...
fn default_parse_args(mode: Mode) -> Vec<String> {
    match mode {
        Mode::Cuda => {
            let Some(cuda) = cuda_path() else {
                warn!(
                    "no CUDA toolkit found (set CUDA_PATH, or install to /usr/local/cuda, \
                     /opt/cuda or /usr/lib/cuda); kernel launch edges will not resolve"
                );
                return ["-x", "cuda", "--cuda-host-only", "-std=c++17"]
                    .into_iter()
                    .map(str::to_string)
                    .collect();
            };
