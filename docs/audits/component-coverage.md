...
| `hades-proto` | 2 / 1 | Build script compiles four proto sources into client/server code; library exports common, embedding, extraction and training modules. Generated-type tests and service/client contracts exercise selected compatibility. | Full error/status, size-limit and client/provider version compatibility matrix. Generated stubs alone do not establish a running provider. |
